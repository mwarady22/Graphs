...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        double size;
        String endbit = ".\n";
        while (currentlocationindex < segmentsize - 1) {
            Road road0 = _map.getLabel(segment.get(currentlocationindex),
                    segment.get(currentlocationindex + 1));
            size = road0.length();
            if (currentlocationindex + 1 < segmentsize - 1) {
                Road road1 = _map.getLabel
                        (segment.get(currentlocationindex + 1),
                        segment.get(currentlocationindex + 2));
                while ((road0.direction().equals(road1.direction()))
                        && (road0.toString().equals(road1.toString()))) {
                    size += road1.length();
            ...
            if (currentlocationindex == segmentsize - 2) {
                endbit = String.format(" to %s.\n",
                        _map.getLabel(segment.get(currentlocationindex
                                + 1)).toString());
            }
            DecimalFormat numForm = new DecimalFormat("0.0");
            System.out.printf("%s. Take %s %s for %s miles%s",
                    seq, road0.toString(), road0.direction().fullName(),
                    numForm.format(size), endbit);
            currentlocationindex += 1;
            seq += 1;
        }
        return seq;
