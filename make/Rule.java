...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException(
                    "There is already a command more cannot be added.");
        } else {
    ...
    private void checkFinishedDependencies() {
        Iteration nextiterator = _depends.successors(_vertex);
        while (nextiterator.hasNext()) {
            if (_depends.getLabel((int) nextiterator.next()).isUnfinished()) {
                error("error child is not finished, cannot be built");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        Iterator nextiterator = _depends.successors(_vertex);
        int thistime = _depends.getLabel(_vertex).getTime();
        while (nextiterator.hasNext()) {
            int next = (Integer) nextiterator.next();
            if (_depends.getLabel(next).getTime() > thistime) {
                return true;
            }
        }
        return false;
