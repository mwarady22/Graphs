...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        clear();
        _fringe.removeAll(_fringe);
        _fringe.addAll(V0);
        while (!_fringe.isEmpty()) {
            int vertex = _fringe.poll();
            if ((vertex < 0) && (!postVisit(Math.abs(vertex)))) {
                return;
            }
